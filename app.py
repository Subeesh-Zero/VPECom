...
# ==========================================
# 🎨 SCREEN 1: SETUP (FULL SYNC MODE)
# ==========================================
SETUP_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>Shop Setup (Full Sync)</title>
...
<body>
    <div class="card">
        <h2>🚀 Full Standard Connect</h2>
        <p>This will download all files to ensure 100% sync.</p>
        <input type="text" id="repoUrl" placeholder="Paste GitHub Repo Link Here...">
        <button onclick="startSetup()" id="btn">Connect & Download All</button>
    ...
    <script>
        async function startSetup() {
            ...
            document.getElementById('msg').innerText = "Downloading Repository... (Wait)";
            try {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .preview-box { flex: 0 0 60px; height: 60px; border-radius: 6px; overflow: hidden; position: relative; border: 1px solid #ddd; background: #eee; }
...
<body>
    ...
    <div class="main">
        <div id="dashboard" class="section active">
            <div class="header"><div class="page-title">Dashboard</div></div>
            <div class="form-grid">
    ...
    <script>
        ...
        async function saveMultiProduct() {
            ...
            Swal.fire({ title: 'Uploading...', text: 'Syncing to Git...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
...
# ==========================================
# 🚀 SMART PUSH (STANDARD FULL SYNC)
# ==========================================
def smart_push(msg):
    try:
        print(f"Syncing: {msg}")
        
        # 1. Add Everything (Works perfectly in full sync mode)
        subprocess.run(["git", "add", "."], check=True)
        
        # 2. Commit
        subprocess.run(["git", "commit", "-m", msg], check=False)
        
        # 3. Pull (To avoid conflicts)
